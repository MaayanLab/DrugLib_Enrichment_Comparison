...
subtitle: "Setup and dataset description"
author: "Damon Pham"
...
Now that all gene vector matrices have been created, here is a summary of the features of each library. (Note: the union and intersection gvms between Drug Bank and Target Central are not included in this table.)
...
From the "average number of genes per sample" column, it is apparent that the gene set sizes for samples from DrugBank, TargetCentral, DGIdb and DrugRepHub are small. These libraries will be expanded using PPI data from three sources: ARCHS4, hu.MAP and BioGRID. To do this, the PPI libraries will be narrowed down such that each gene/protein in the library has up to 100 top coexpressed genes/proteins, including itself. (For BioGRID, since coexpression scores are not provided, the first 100 coexpression entries for each gene/protein, from top to bottom in the original file, were kept.) Then, each gene set in the drug libraries is expanded by adding to it the coexpression entries of each gene/protein it contains. For example, the set of genes {CTPS, MLLT11} will become the union of the coexpression sets for CTPS and MLLT11. (If a gene doese not appear in the PPI library, the coexpression set is interpreted as just the gene itself.) This results in three new drug-gene libraries for each original drug-gene library: one for each PPI library used to expand it. 
