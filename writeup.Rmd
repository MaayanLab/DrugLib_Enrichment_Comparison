...
list.of.packages <- c('kableExtra', 'knitr', 'ggplot2', 'cmapR', 'stringi', 'tidyr', 'data.table','gridExtra','reshape2','grid', 'biomaRt', 'webchem', 'httr',  'jsonlite','UniProt.ws', 'magrittr', 'gplots', 'UpSetR')
...
In order to perform enrichment analysis, I needed to do three things:
...
2. Identify matching annotations, i.e. drug synonyms, among the libraries. I accomplished this by renaming each annotation to its corresponding drug's PubChem Compound ID. This was done by using the webchem R package to query PubChem's synonym database with the drug name(s) or different database ID(s) for each annotation. If I could not obtain a PubChem Compound ID, I instead used the drug's common name, with alphanumeric characters only, and all letters in lowercase.

3. Expand the gene sets of the drug target libraries. Each drug only targets a few genes directly, but through gene-gene interactions they often indirectly regulate the expression of many more genes. I tried to capture the net effect of each drug by adding to its gene set the 100 most interacting or co-expressed genes, for each gene in the original set.

These steps are described below. I first complete steps (1) and (2) for each library. Then, I summarize their properties and complete step (3) before performing enrichment analysis. 

#### DrugBank, TCRD, DrugRepHub, DGIdb, and DrugCentral

For each drug, these five drug target libraries have a name and ChEMBL ID (always present), as well as a DrugBank ID and SMILES (sometimes missing). I began by aggregating all drugs across these libraries. Then, I tried to obtain the PubChem IDs by using, in this order:

* the ChEMBL ID
* the Drug Name
* the DrugBank ID, if any
* the SMILES, if any
...
I was unable to obtain PCIDs for `r sum(is.na(PCIDs))` out of `r length(PCIDs)` annotations.

Next, for each library, I checked for compounds which have different ChEMBL IDs but were assigned the same PubChem ID.
...
Now that I obtained the PubChem IDs, I transformed the libraries into gene vector matrices and used the PubChem IDs (or the drug's common name if the PubChem ID was not found) as the column annotation names. 
...
Drug-protein interactions are given a Confidence Score from 150 to 1000, with higher numbers indicating higher probability of intereaction. Here is a table summarizing the drug sets in STITCH after applying different cutoffs for interaction Confidence Scores.
...
kable_styling(kable(info, 'html', caption='Table 1: Summary statistics for STITCH interactions with different Confidence Score cutoffs. For a given cutoff value, I removed all interactions with a Confidence Score lower than this value. I then counted the number of chemicals which appear in this new subset (i.e. have at least one interaction with a Confidence Score which made the cutoff), and measured their average number of interactions.', digits=2, font_size=10))

rm(STITCH, gene_symbol_lookup, gene_symbol_lookup_fname, info, s, cutoffs)
```

I decided to make gmts and gvms for cutoff levels 500, 600, 700 and 800. To be consistent with the five previous libraries, I only kept chemicals with at least five interactions afer applying the cutoff. I also made gmts and gvms with proteins whose ENSP ID could not be converted to a HGNC name removed. These are what I will use for enrichment analysis.
...
* `activity_comments`: only keeping interactions with comments that confirmed activity, or which I was unsure how to interpret. This means I removed interactions with negative or inconclusive activity, or which were blank.
...
DTCommons = fread(DTCommons_intermediate.fname, sep='\t', showProgress=FALSE)
```
...
I was unable to obtain PCIDs for `r length(unique(names(PCIDs)[is.na(PCIDs)]))` out of `r length(unique(names(PCIDs)))` annotations.

```{r}
rm(activity_comments, uniprot_to_hgnc, up, no_activity, uncertain, PCID_fname, unique_target_ids)
```

For the final gvm name I used the PCID, or the drug name if that was unavailable, or the ChEMBL ID if both were unavailable.
...
*CREEDS*: In CREEDS, the annotation of each sample contains both a name and a DrugBank ID. In addition, some names have a synonym in parentheses. I began by extracting and scrubbing the drug name and DrugBank ID. I then removed embedded synonyms from the drug name.
...
I tried to obtain the PubChem IDs by using the following information, in this order:
...
Out of `r length(CREEDS_unique)` unique drugs, I was unable to obtain  `r length(CREEDS_noPCID)` PCIDs. I used the drug name if the PCID could not be found. 
...
*LINCS*: I extracted the drug names from the sample annotations. I then removed embedded synonyms.
...
LINCS_identifiers = unique(LINCS_identifiers[,-1])
```

I was unable to obtain PCIDs for `r sum(is.na(LINCS_identifiers$PCIDs))` out of `r length(LINCS_identifiers$PCIDs)` drugs. I used the drug name if the PCID could not be found.
...
```{r}
system('python summarize_libraries.py')
```

*Summary statistics*: Here is a table summarizing the features of each drug library.
...
colnames(t1) = c('Name', 'Library Type', 'Description','Samples', 'Unique drugs', 'Targets', 'Average targets per annotation', 'Data preprocessing')

t1[1:10,'Samples'] = NA
t1[,'Name'] = paste0("[", as.character(t1[,'Name']), "](http://", links, ")")

kable(t1, 'html', caption='Table 2: Summary of all drug libraries. For the drug target libraries, I counted the number of drugs and protein targets, and measured the average number of protein targets per annotation (i.e. drug). For the drug perturbation libraries, I counted the number of samples and protein targets, and measured the average number of protein targets per annotation (i.e. sample). I also counted the number of drugs, since there are often multiple samples for the same drug.') %>%
  kable_styling(bootstrap_options=c('striped', 'condensed', full_width=F, font_size=10))

```

*Comparison of drug sets*: Each library measures different populations of drugs. I first looked at the size of the intersection between each drug target library with each drug perturbation library, to see if my enrichment analysis benchmarking method would be feasible.

```{r eval=TRUE}
t2 = read.csv('intermediate_files/lib_summary_drugs.csv', sep='\t', header=FALSE, colClasses=c('character','character'))

t2.list = strsplit(t2[,2], split='\\|')
names(t2.list) = t2[,1]
t2.list = t2.list[-match(c('STITCH_600cutoff','STITCH_700cutoff'), names(t2.list))]

all.drugs = vector(mode='character')
for(i in 1:length(t2.list)){
  all.drugs = union(all.drugs, t2.list[[i]])
}

overlaps = matrix(nrow = length(t2.list) - 2, ncol = 2)
colnames(overlaps) = c('LINCS','CREEDS')
row.names(overlaps) = names(t2.list)[-match(c('LINCS','CREEDS'), names(t2.list))]
for(i in 1:nrow(overlaps)){
  for(j in 1:ncol(overlaps)){
    overlaps[i,j] = length(intersect(t2.list[[row.names(overlaps)[i]]], t2.list[[colnames(overlaps)[j]]]))
  }
}

t2.list = t2.list[-match(c('STITCH_800cutoff'), names(t2.list))]

t2.grid = matrix(0, nrow = length(all.drugs), ncol=length(t2.list))
row.names(t2.grid) = all.drugs
colnames(t2.grid) = names(t2.list)
for(i in 1:length(t2.list)){
  t2.grid[,i] = ifelse(all.drugs %in% t2.list[[i]], 1, 0)
}
```

```{r eval=TRUE}
kable_styling(kable(overlaps, 'html', font_size=10, caption='Table 3: Size of drug set intersections between drug target libraries and drug perturbation libraries.')) %>%
  kable_styling(bootstrap_options=c('striped', 'condensed', full_width=F, font_size=10))
```

In my opinion, only the intersection set between TargetCentral and CREEDS is small enough to raise concern.

In addition to looking at these pairwise intersection sizes, I also looked at the overall similarities of each library using a heatmap with hierarchical clustering.

```{r fig.cap='Figure 1: Heatmap and hierarchical clustering of drug libraries. Drugs are listed along the rows (unlabeled). The grid is white where the drug appears in the library, red otherwise. Cyan lines separate red regions from white. To make the library dendrogram clusterings, Jaccard similarity (intersection size over union size) is measured between each library pair. Then, a clustering algorithm (complete linkage) sorts the libraries into hierarchical clusters. The drug dendrogram clusterings are computed similarly, but I do not analyze these.', eval=TRUE}
heatmap.2(t2.grid[rowSums(t2.grid) > 1,], labRow=rep('', length(t2.list)), 
          distfun = function(x) dist(x,method = 'binary'), margins=c(10,1))
```

STITCH and Drug Target Commons, the largest two libraries, share many drugs which are not represented in the other libaries. They are also the most similar to LINCS and CREEDS, the drug perturbation signature libraries. The five other drug target libraries appear in a separate cluster at the first hierarchy. 

Finally, I used the UpSet R package to visualize the frequency of set memberships between the libraries. Here are the top 40 set memberships:

```{r fig.cap='Figure 2: Largest intersection sets between the drug libraries. The size of each cell in a 9-way Venn Diagram was measured, and the largest 40 were plotted. For example the right-most set contains the drugs in TargetCentral and DrugCentral, but not in any other library.', eval=TRUE}
upset(fromList(t2.list), nsets=length(t2.list), nintersects=40, scale.intersections='log2', order.by = 'freq')
```

It is most common for drugs to be included in only one of the larger libraries. For drugs that belong to multiple libraries, one of the libraries they belong to is often either STITCH or DTCommons. TargetCentral, and CREEDS to a lesser extent, is not well represented within the largest intersection sets. 

#### Expansion with coexpression or PPI data

The average gene set sizes for the drug target libraries (DrugBank, TargetCentral, DGIdb, DrugRepHub, DrugCentral, DTCommons, and STITCH) are too small for enrichment analysis. These libraries were expanded using data from three sources: ARCHS4 (a gene coexpression database), hu.MAP, and BioGRID (both PPI databases). 

To do this, coexpression/interaction sets were extracted from each library by taking the top 100 coexpressed/interacting genes/proteins for each gene/protein. (For BioGRID, since interaction scores are not provided, the first 100 entries for each protein, from top to bottom in the original file, were kept.) Then, each gene set in the drug libraries is expanded by adding to it the coexpression/interaction set of each gene in the original set. For example, the gene set {CTPS, MLLT11} becomes the union of: (1) {CTPS, MLLT11}, (2) the coexpression/interaction set for CTPS, and (3) the coexpression/interaction set for MLLT11. (If a gene does not appear in the expansion library, the coexpression/intereaction set is interpreted as just the gene itself.) The result is three new drug target libraries for each original drug target library: one for each expansion library.
...
Below are histograms showing how the distributions of gene set sizes change after expansion with protein co-expression or interaction data. 

```{r eval=TRUE, fig.cap='Figure 3: Histograms of gene set sizes before and after expansion for each of the drug libraries.'}
libs = c('STITCH_800cutoff_5OrMoreTargets_noENSP',
...
for(i in 1:length(libs)){
  lib = libs[[i]]
  t = data.frame(t(read.csv(paste0(c(
    'intermediate_files/lib_summary_geneset_sizes_', lib, '.csv'), collapse=''),
    sep='\t', row.names=1)))
  
  h = gather(t, library, geneset_size)
  
  plots[[i]] = ggplot(h, aes(x = geneset_size, fill = library)) +
  geom_histogram(alpha=0.8, position='identity', bins=50) + scale_x_log10() + 
    labs(title=gsub('_noENSP', '', gsub('_5OrMoreTargets', '', lib))) + 
    theme(legend.position = ifelse(i==length(libs), 'right', 'none'),
...
grid.arrange(grobs=plots, ncol=3, layout_matrix=lay.matrix)
```

## Enrichment analysis

After creating the expanded libraries, I performed enrichment analysis, using the Fisher's exact test, between each drug target library and drug perturbation signature library pair.
...
Here are the results using the drug perturbation library gene sets as inputs, and the drug target libraries as the gene sets which are being ranked. (I was unable to obtain results for inputting STITCH or DTCommons into LINCS due to lack of time.)

![Figure 4: Enrichment analysis results. First, the samples in the drug perturbation signature library whose drug also appears in the drug target library are identified. For each of these samples, enrichment analysis compares its gene set to those of the drug target library to rank the drug target library's drugs from most to least enriched. A bridge plot is created to show the distribution of ranks for the matches, i.e. the gene sets corresponding to the same drug which was inputted. The legend gives the area under the curve (AUC) of the bridge.](figures/druglib_bridgeplot.png)

Here is a zoomed-in view of the results, looking at only the top 15% ranked terms. 

![Figure 5: Enrichment analysis results, top 15th percentile. This is the same as Figure 3, but zoomed in to the left 15% of the graph.](figures/druglib_bridgeplot_15th_pctile.png)