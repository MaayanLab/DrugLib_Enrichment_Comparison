...
list.of.packages <- c('kableExtra', 'knitr', 'ggplot2', 'data.table','gridExtra','reshape2','grid')
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
lapply(list.of.packages, require, character.only = TRUE)
rm(list.of.packages, new.packages)

knitr::opts_chunk$set(cache=TRUE, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE, fig.align="center")
```

## Project description

In this project, I evaluate the accuracy of various drug target libraries by measuring their agreement with different drug perturbation signature libraries. The drug target libraries I look at are DrugBank, the Target Central Resource Database, the Drug Repurposing Hub, the Drug Gene Interaction Database (DGIdb), and DrugCentral. The drug perturbation signature libraries I used are the CRowd Extracted Expression of Differential Signatures (CREEDS) and the LINCS L1000. For all the drug target libraries, since their average number of gene targets was small, I used protein-protein interaction (PPI) databases to create expanded gene sets from their target lists. The PPI databases I use are hu.MAP, ARCHS4, and BioGRID.

I measured agreement using enrichment analysis--specifically, the Fisher's exact test for overrepresentation. I treat each (drug, expanded target list) from the drug target libraries as an (annotation, gene set) pair, and each (drug sample, list of most-perturbed genes) from the drug perturbation signature libraries also as an (annotationn, gene set) pair. I then then perform enrichment into one library using the annotated gene sets from the other library, and observe how highly the matching annotations, i.e. those which correspond to the same drug as the input, are ranked. If two libraries agree with one another, then these matching annotations should often be highly ranked.
...
Each drug perturbation signature library was originally two separate gmt files: one for the up-perturbed genes, and one for the down-perturbed genes. I merged these files by taking the union of up-perturbed and down-perturbed genes for each sample. I then converted this to a gene vector matrix with each sample as its own column.

```{r get_drug_pert_sig_gvms}
...
This next step consists of two tasks: separating drug names from the sample names in the perturbation signature libraries, and then identifying drug synonyms. Synonyms were either provided by the libraries as a separate column or file, or embedded in the drug name itself, e.g. "chembl1255588 ((+)-butaclamol)". Once I accomplished these tasks, I returned to each library and re-labeled each annotation with its corresponding drug's lowest alphabetical synonym. 

To match identical drug names with different punctuation, I only kept alphanumeric characters and lowercased all letters. 

Here is a summary of my procedure:

* Create a synonym table with two columns: one for drug names, and one for IDs. Drugs that are synonyms will have the same ID. 
* For each library: 
    * Add synonyms given as separate columns or files to the list. 
    * Extract the drug names from the sample names, if necessary.
    * Check the drug names for embedded synonyms. If any are present, add the true drug name and its synonym to the list. Then remove the embedded synonym from the drug name, For example, "chembl1255588 ((+)-butaclamol)" will become "chembl1255588" and ('chembl1255588', 'butaclamol') will be added to the synonym table. 
    * Save the list of drug names for later.
* After all synonyms across all libraries have been identified, go back and label each sample with its drug's lowest alphabetical synonym.
...
```{r}
expand_synonym_table = function(input_table, synonym_table){
  #Collects the synonyms from `input_table` and adds them to `synonym_table`
  #input_table : a data.table of at least two columns, where each row contains a set of synonyms. Empty values are allowed.
  #synonym_table : a data.table of synonyms. The first column is the name or term, and the second column is its id. Synonyms have the same id. e.g. data.table(cbind(c('dog','cat','canine'),c(1,2,1))).

  names = synonym_table[['name']]
  ids = synonym_table[['id']]
  ...
  for(r in 1:nrow(input_table)){
    ...
    #If at least one of these names is already in the synonym table:
    is_rep = row %in% names
    if(any(is_rep)){
  ...
  synonym_table = data.table(cbind(names, ids))
  synonym_table = synonym_table[, ids := as.numeric(ids)]
  colnames(synonym_table) = c('name','id')
  return(synonym_table)
}
```

```{r}
extract_embedded_synonyms = function(drug_names, synonym_table, left_paren_split = ' \\(', no_syns_substrs = NULL){
  #Identifies drug names containing a synonym enclosed in parentheses. Adds the synonym pairs to the list, and replaces the drug names with the true drug names. The true drug name is defined as the substring to the left of `left_paren_split`, and the synonym is defined as the substring between `left_paren_split` and the right-most ")". For example, the drug name "chembl1255588 ((+)-butaclamol)" will become "chembl1255588" and ('chembl1255588', 'butaclamol') will be added to the synonym table. 
  #Returns the new drug names and the expanded synonym table. 
  ...
  for(i in 1:length(drug_names.might_have_synonym)){
    ...
    if(all(nchar(syns) > 2)){
      ...
      synonym_table = expand_synonym_table(input_table, synonym_table)
  ...
  return(list(drug_names, synonym_table))
}
```

*Drug target libraries*: These libraries had a drug name, a ChEMBL ID, and a DrugBank ID for each drug. I began by looking at the drug names and checking them for embedded synonyms. I did find a few; they seemed to be enclosed in parentheses and separated by a space. Below is a list of drug names in these libraries containing the " (" substring.

```{r drug_target_libraries_show_potential_synonyms, include=TRUE}
...
kable(potential_synonyms, 'html', caption='Potential embedded synonyms') %>%
...
for(i in 1:length(drug_target_libnames)){
  ...
  paren_results = extract_embedded_synonyms(lib$DrugName, synonym_table=drug_synonyms, no_syns_substrs = c('methyl \\(','orphenadrine \\(chloride\\)'))
  ...
  drug_synonyms = expand_synonym_table(input_table, drug_synonyms)
...
*CREEDS*: In CREEDS, each sample is described by both a name and a DrugBank ID. In addition, some names contain a symnonym in parentheses. I began by separating the drug name and DrugBank ID from the sample name, and hand-scrubbing the drug name. 
...
paren_results = extract_embedded_synonyms(CREEDS.drugs, drug_synonyms)
...
#Change 'na' to blank (will be removed from synonym table) and append 'db' where absent.
...
CREEDS.drug_and_DBname = matrix(c(CREEDS.drugs, CREEDS.DB_ID), ncol=2)
CREEDS.drug_and_DBname = unique(CREEDS.drug_and_DBname)
drug_synonyms = expand_synonym_table(CREEDS.drug_and_DBname, drug_synonyms)
save(drug_synonyms, file='synonyms\\drug_synonyms2.r')

rm(drug, i, paren_results, CREEDS.drug_and_DBname, CREEDS.DB_ID, CREEDS)
```
...
paren_results = extract_embedded_synonyms(LINCS.drugs, drug_synonyms, 
...
drug_synonyms = expand_synonym_table(add_this, drug_synonyms)
...
After collecting all the synonyms, I appended to each annotation the highest-alphabetical synonym of the drug it represents, and separated it with three vertical bars. For example, in CREEDS, the sample "vitamin c DB00126 mouse GSE32994 sample 3510" became "ascorbate|||vitamin c DB00126 mouse GSE32994 sample 3510".
...
replace_with_synonyms = function(drug_name_vector, drug_synonyms, original_name_vector=NULL){
  ...
  #drug_synonyms: the synonym table
  #original_name_vector: the original sample names
  drug_name_vector = clean_drug_names(drug_name_vector)
...
if(!file.exists('original_drug-gene_libs/CREEDS_Drugs_gvm2.csv')){
...
if(!file.exists('original_drug-gene_libs/LINCS_L1000_Chem_Pert_gvm2.csv')){
...
for(i in 1:length(drug_target_libnames)){
  lib = fread(drug_target_libnames[[i]])
  result = replace_with_synonyms(dt_drugnames_nosyns[[i]], drug_synonyms, lib$DrugName)
  lib$DrugName_Final = result
  write.csv(lib, file=drug_target_libnames[[i]], quote=FALSE, sep=',', row.names=FALSE)
}

rm(drug_synonyms, lib, drug_target_libnames, dt_drugnames_nosyns, i, result)
```

#### Creating gvms for the drug perturbation signature libraries

Finally, I converted the drug target libraries to gvms. I combined all drug targets for the same drug into a single gene set, and used its re-labeled annotation as the column label.

```{r}
system('python get_drugtarget_gvms.py')
```

#### Summary of all drug libraries

After creating all the gene vector matrices, I made a table to summarize the features of each library.

```{r read_gvms, results='hide'}
DrugBank = fread('original_drug-gene_libs/1_DrugBank_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_gvm.csv')
TargetCentral = fread('original_drug-gene_libs/2_TargetCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_gvm.csv')
DrugCentral = fread('original_drug-gene_libs/5b_DrugCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_Human_1-14-18_gvm.csv')
DGIdb = fread('original_drug-gene_libs/4_DGIdb_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_gvm.csv')
DrugRepHub = fread('original_drug-gene_libs/3_RepurposeHub_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_gvm.csv')
