...
list.of.packages <- c('kableExtra', 'knitr', 'ggplot2', 'cmapR', 'stringi', 'data.table','gridExtra','reshape2','grid', 'biomaRt', 'webchem', 'httr', 'jsonlite','UniProt.ws', 'magrittr', 'gplots', 'UpSetR')
...
kable_styling(kable(info, 'html', caption='Table 1: Summary statistics for STITCH interactions with different combined score cutoffs', digits=2, font_size=10))
...
I was unable to obtain PCIDs for `r sum(is.na(LINCS_identifiers$PCIDs))` out of `r length(LINCS_identifiers$PCIDs)` annotations. I used the drug name if the PCID could not be found.
...
kable(t1, 'html', caption='Table 2: Summary of all drug libraries') %>%
  kable_styling(bootstrap_options=c('striped', 'condensed', full_width=F, font_size=10))
...
t2.list = t2.list[-match(c('STITCH_500cutoff'), names(t2.list))]
...
```{r}
kable_styling(kable(overlaps, 'html', font_size=10, caption='Size of drug set intersections between drug target libraries and drug perturbation libraries'))
```
...
```{r fig.cap='Figure 1: Heatmap and hierarchical clustering of drug libraries', eval=TRUE}
heatmap.2(t2.grid[rowSums(t2.grid) > 1,], labRow=rep('', length(t2.list)), margins=c(10,1))
```

STITCH and Drug Target Commons, the largest two libraries, share many drugs which are not represented in the other libaries. TargetCentral, CREEDS and LINCS seem to be similar to each other, with DGIdb, DrugCentral, DrugBank and RepurposeHub forming the other cluster. 
...
```{r fig.cap='Figure 2: Largest intersection sets between the drug libraries', eval=TRUE}
...
Here are the results I obtained so far:

![Figure 3: Enrichment analysis results](figures/druglib_bridgeplot.png)