...
```{r}
CREEDS_unique = unique(CREEDS_identifiers[,c('drug','PCID')])
```

Out of `r nrow(CREEDS_unique)` unique drugs, I was unable to obtain  `r sum(is.na(CREEDS_unique$PCID)) ` PCIDs. 
...
```{r}
nrow=1
DrugBank = names(fread('gvms/1_DrugBank_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
TargetCentral = names(fread('gvms/2_TargetCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
RepurposeHub = names(fread('gvms/3_RepurposeHub_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
DGIdb = names(fread('gvms/4_DGIdb_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
DrugCentral = names(fread('gvms/5b_DrugCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_Human_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]

DTC = names(fread('gvms/drugtargetcommons_5ormoretargets_gvm.csv', nrow=nrow))[-1]

STITCH500 = names(fread('gvms/STITCH_500cutoff_gvm.csv', nrow=nrow))[-1]
STITCH600 = names(fread('gvms/STITCH_600cutoff_gvm.csv', nrow=nrow))[-1]
STITCH700 = names(fread('gvms/STITCH_700cutoff_gvm.csv', nrow=nrow))[-1]
STITCH800 = names(fread('gvms/STITCH_800cutoff_gvm.csv', nrow=nrow))[-1]

LINCS = names(fread('gvms/LINCS_L1000_Chem_Pert_PCIDs_gvm.csv', nrow=nrow))[-1]
CREEDS = names(fread('gvms/CREEDS_PCIDs_gvm.csv', nrow=nrow))[-1]
```
