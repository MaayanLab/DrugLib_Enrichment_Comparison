...
for(i in 1:length(fivedtlibs_fnames)){
  ...
  if(nrow(multimapped_rows)>1){
    print(knitr::kable(multimapped_rows, caption=fivedtlibs_libnames[[i]], 'html') %>% kable_styling())
  }
...
CREEDS_noPCIDs_fname = 'intermediate_files/CREEDS_noPCIDs_gvm.csv'
#Extract drug names and the DrugBank IDs. 
CREEDS = fread(CREEDS_noPCIDs_fname)
...
CREEDS_identifiers[match(CREEDS.synonyms[,1], CREEDS.drugs),'syn'] = CREEDS.synonyms[,2]
rm(CREEDS.drugs, CREEDS.synonyms, CREEDS.DB_ID, embedded, CREEDS.samples, drug, i)
```
...
```{r}
CREEDS.PCID_file = 'synonyms/CREEDS_identifiers.txt'
if(!file.exists(CREEDS.PCID_file)){
  ...
  PCIDs[is.na(PCIDs)] = do_chunkwise(get_cid, drug_name[is.na(PCIDs)], 
                                          out_fname = 'synonyms/CREEDS_temp1.txt', first=TRUE)
  PCIDs[is.na(PCIDs)] = do_chunkwise(get_cid, dbID[is.na(PCIDs)], 
                                          out_fname = 'synonyms/CREEDS_temp2.txt', first=TRUE)
  PCIDs[is.na(PCIDs)] = do_chunkwise(get_cid, synonym[is.na(PCIDs)], 
                                          out_fname = 'synonyms/CREEDS_temp3.txt', first=TRUE)
  PCIDs[is.na(PCIDs)] = do_chunkwise(get_cid, drug_name.cleaned[is.na(PCIDs)], 
                                          out_fname = 'synonyms/CREEDS_temp4.txt', first=TRUE)
  PCIDs[is.na(PCIDs)] = do_chunkwise(get_cid, synonym.cleaned[is.na(PCIDs)], 
                                          out_fname = 'synonyms/CREEDS_temp5.txt', first=TRUE)
  
  write_table(data.frame(c1 = drug_name, c2 = PCIDs), fname=CREEDS.PCID_file, na='')
...
Out of `r length(CREEDS_identifiers$PCID)` unique drugs, I was able to obtain  `r length(CREEDS_identifiers$PCID[!is.na(CREEDS_identifiers$PCID)])` PCIDs. 

```{r replace_CREEDS_names}
CREEDS_fname = 'gvms/CREEDS_gvm.csv'
if(!file.exists(CREEDS_fname)){
  colnames(CREEDS)[-1] = CREEDS_identifiers$gvm
  CREEDS[CREEDS=='TRUE'] = 'True'
  write_gvm(CREEDS, file = CREEDS_fname)
}
...
LINCS_identifiers = data.table(drug = unique(LINCS.drugs))
...
```{r}
LINCS_identifiers$PCIDs[is.na(LINCS_identifiers$PCIDs)] = do_chunkwise(get_cid,  LINCS.drugs[is.na(LINCS_identifiers$PCIDs)], out_fname ='synonyms/LINCS_PCIDs1.txt', DELETE_OUTFILE = FALSE, from='name', first=TRUE)
LINCS_identifiers$PCIDs = as.integer(LINCS_identifiers$PCIDs)

LINCS_identifiers$PCIDs[is.na(LINCS_identifiers$PCIDs)] = do_chunkwise(get_cid,  clean_drug_names(LINCS.drugs[is.na(LINCS_identifiers$PCIDs)]), out_fname ='synonyms/LINCS_PCIDs2.txt', DELETE_OUTFILE = FALSE, from='name', first=TRUE)
LINCS_identifiers$PCIDs = as.integer(LINCS_identifiers$PCIDs)
...
LINCS_identifiers$PCIDs[is.na(LINCS_identifiers$PCIDs)] = LINCS.syns.query.PCIDs[match(LINCS_identifiers$syn, LINCS.syns.query)][is.na(LINCS_identifiers$PCIDs)]
```
...
Out of `r length(LINCS_identifiers$PCIDs)` unique drugs, I was able to obtain all but  `r sum(is.na(LINCS_identifiers$PCIDs))` PCIDs. 

```{r replace_LINCS_names}
LINCS_fname = 'gvms/LINCS_gvm.csv'
if(!file.exists(LINCS_fname)){
  gvm_names = LINCS_identifiers$gvm[match(LINCS.drugs, LINCS_identifiers$drug)]
  colnames(LINCS)[-1] = gvm_names
  LINCS[LINCS=='TRUE'] = 'True'
  write_gvm(LINCS, file= LINCS_fname)
}
```
...
```{r}
nrow=1
DrugBank = names(fread('original_drug-gene_libs/1_DrugBank_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
TargetCentral = names(fread('original_drug-gene_libs/2_TargetCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
RepurposeHub = names(fread('original_drug-gene_libs/3_RepurposeHub_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
DGIdb = names(fread('original_drug-gene_libs/4_DGIdb_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]
DrugCentral = names(fread('original_drug-gene_libs/5b_DrugCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_Human_1-14-18_PCIDs_gvm.csv', nrow=nrow))[-1]

DTC = names(fread('original_drug-gene_libs/drugtargetcommons_5ormoretargets_gvm.csv', nrow=nrow))[-1]

STITCH500 = names(fread('original_drug-gene_libs/STITCH_500cutoff_gvm.csv', nrow=nrow))[-1]
STITCH600 = names(fread('original_drug-gene_libs/STITCH_600cutoff_gvm.csv', nrow=nrow))[-1]
STITCH700 = names(fread('original_drug-gene_libs/STITCH_700cutoff_gvm.csv', nrow=nrow))[-1]
STITCH800 = names(fread('original_drug-gene_libs/STITCH_800cutoff_gvm.csv', nrow=nrow))[-1]

LINCS = names(fread('original_drug-gene_libs/LINCS_L1000_Chem_Pert_PCIDs_gvm.csv', nrow=nrow))[-1]
CREEDS = names(fread('original_drug-gene_libs/CREEDS_PCIDs_gvm.csv', nrow=nrow))[-1]
```

```{r}
PCIDs = list(CREEDS, DGIdb, DrugBank, DrugCentral, DTC, LINCS, RepurposeHub, 
            STITCH700, TargetCentral)
names(PCIDs) = c('CREEDS','DGIdb','DrugBank','DrugCentral','DTC','LINCS','RepurposeHub',
                 'STITCH700','TargetCentral')

PCIDs = PCIDs[1:5]
venn.plot <- venn.diagram(
   x = PCIDs,
filename = "PubChemmID_Venn.tiff",
col = "black",
lty = "dotted",
lwd = 4,
alpha = 0.50,
...
```{r}
#drugs = list(DrugBank, TargetCentral, RepurposeHub, DGIdb, DrugCentral, STITCH600, LINCS, CREEDS)
dtarget = list(DGIdb, DrugBank, DrugCentral, DTC, RepurposeHub, STITCH500, STITCH600, STITCH700, STITCH800, TargetCentral)
dperturb = list(CREEDS, LINCS)

n_matches = matrix(NA, length(dtarget), length(dperturb))
for(i in 1:length(dtarget)){
  for(j in 1:length(dperturb)){
    lib.i = dtarget[[i]]
    lib.j = dperturb[[j]]
    
    n_matches[[i,j]] = length(intersect(lib.i, lib.j))
  }
}

Total = sapply(dtarget, length)

rownames(n_matches) = c('DGIdb','DrugBank','DrugCentral','DTC','RepurposeHub','STITCH500','STITCH600','STITCH700','STITCH800','TargetCentral')
colnames(n_matches) = c('CREEDS','LINCS')

t = cbind(n_matches, Total)

kable_styling(kable(t, 'html', caption='Number of matches'))
```

```{r}
nrow=-1
DrugBank = fread('original_drug-gene_libs/1_DrugBank_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow)
TargetCentral = fread('original_drug-gene_libs/2_TargetCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow)
RepurposeHub = fread('original_drug-gene_libs/3_RepurposeHub_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow)
DGIdb = fread('original_drug-gene_libs/4_DGIdb_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_1-14-18_PCIDs_gvm.csv', nrow=nrow)
DrugCentral = fread('original_drug-gene_libs/5b_DrugCentral_Column_5OrMoreTargets_MissingChEMBL_IDsRemoved_Human_1-14-18_PCIDs_gvm.csv', nrow=nrow)

#STITCH500 = fread('original_drug-gene_libs/STITCH_500cutoff_gvm.csv', nrow=nrow)
STITCH600 = fread('original_drug-gene_libs/STITCH_600cutoff_gvm.csv', nrow=nrow)
#STITCH700 = fread('original_drug-gene_libs/STITCH_700cutoff_gvm.csv', nrow=nrow)
#STITCH800 = fread('original_drug-gene_libs/STITCH_800cutoff_gvm.csv', nrow=nrow)

LINCS = fread('original_drug-gene_libs/LINCS_L1000_Chem_Pert_PCIDs_gvm.csv', nrow=nrow)
CREEDS = fread('original_drug-gene_libs/CREEDS_Drugs_PCIDs_gvm.csv', nrow=nrow)
```

```{r table_1, include=TRUE}

libnames = c('DrugBank','TargetCentral','DGIdb','DrugRepHub','DrugCentral','STITCH','Drug Target Commons','CREEDS','LINCS L1000')

full_libnames = c('DrugBank','Target Central Resource Database','Drug Gene Interaction Database','Drug Repurposing Hub','DrugCentral','','Drug Target Commons', 'CRowd Extracted Expression of Differential Signatures','Library of Integrated Network-Based Cellular Signatures')
links = c('drugbank.ca','juniper.health.unm.edu/tcrd','dgidb.org','clue.io/repurposing','drugcentral.org','STITCH.embl.de','amp.pharm.mssm.edu/CREEDS','lincsproject.org')

libtypes = c(rep('Drug Target', 7), rep('Perturbation Signature', 2))
...
organisms_and_n_samples = c(
  ...
  paste0('Human (',as.character(ncol(STITCH) -1), ')'),
  paste0('Human (',as.character(ncol(DTC) -1), ')'),
...
gvms = list(DrugBank,TargetCentral,DGIdb,DrugRepHub,DrugCentral,STITCH,DTC,CREEDS,LINCS)

n_drugs = as.character(sapply(gvms, get_n_drugs))
...
avg_n_genes_per_sample = as.character(sapply(gvms, get_avg_n_genes))
...
colnames(t1) = c('Name, Abbreviated', 'Name, Full', 'Library Type', 'Description','Organism (number of samples)', 'Number of unique drugs', 'Average number of genes per annotation', 'Data preprocessing')
...
To do this, gene coexpression sets were extracted from each PPI library by taking the top 100 coexpressed genes/proteins for each gene/protein. (For BioGRID, since coexpression scores are not provided, the first 100 coexpression entries for each gene/protein, from top to bottom in the original file, were kept.) Then, each gene set in the drug libraries is expanded by adding to it the coexpression set of each gene/protein it contains. For example, the gene set {CTPS, MLLT11} becomes the union of the coexpression sets for CTPS and MLLT11. (If a gene does not appear in the PPI library, the coexpression set is interpreted as just the gene itself.) The result is three new drug target libraries for each original drug target library: one for each PPI library.
...
expansion_libs_names = c('ARCHS4','BioGRID','huMAP')

#gvm_libnames = c('DGIdb','DrugBank','DrugCentral','RepurposeHub','TargetCentral')
#gvms = list(DGIdb, DrugBank, DrugCentral, RepurposeHub, TargetCentral)

gvm_fnames = c('drugtargetcommons_5ormoretargets')
gvm_libnames = c('DTC'),# 'STITCH_500cutoff_noENSP', 'STITCH_600cutoff_noENSP', 
                 #'STITCH_700cutoff_noENSP', 'STITCH_800cutoff_noENSP')
gvms = list(DTCommons)

for(i in 1:length(gvms)){
  gvm=gvms[[i]]
  for(j in 1:length(expansion_libs_names)){
    expansion_lib = expansion_libs[j]
    expansion_lib_name = expansion_libs_names[j]
    output_fname = paste0('expanded_drug-gene_libs/', gvm_libnames[i], '_expanded_with_', expansion_lib_name, '_gvm.csv')
    print(output_fname)
...
![](figures/DRAFT_druglib_bridgeplot.png "Enrichment analysis results")